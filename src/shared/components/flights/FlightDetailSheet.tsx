...
export function FlightDetailSheet({
  ...
  const hasNewLuggage = flight.luggage && flight.luggage.length > 0;
  const hasLegacyLuggage =
    flight.checkedBagKg || flight.carryOnAllowed || flight.personalItemAllowed;
  const hasLuggage = hasNewLuggage || hasLegacyLuggage;
                ...
                <div className="flex items-end gap-3">
                  ...
                  {isSignedIn && onToggleSave && (
                    <button
                      ...
                      aria-label={isSaved ? t("unsaveFlight") : t("saveFlight")}
                    >
                      <Heart
                  ...
                  {hasNewLuggage ? (
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {flight.luggage!.map((item, i) => (
                        <div
                          key={i}
                          className="flex flex-col items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50/50 dark:border-emerald-900/50 dark:bg-emerald-950/20 p-3 text-center"
                        >
                          <Luggage className="size-5 text-emerald-600 dark:text-emerald-400" />
                          <span className="text-[11px] font-medium leading-tight">
                            {luggageTypeLabel(item.type, t)}
                          </span>
                          {item.weightKg && (
                            <span className="text-[10px] text-muted-foreground">
                              {t("luggageWeightKg", { kg: item.weightKg })}
                            </span>
                          )}
                        </div>
                      ))}
